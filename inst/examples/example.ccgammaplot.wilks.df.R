...
#source('~/Dropbox/R-packages/RGENERATEPRECVis/R/ccgammaplot.R', chdir = TRUE)wilks
...
model_multisite_wilks <- PrecipitationOccurenceMultiSiteModel(x=prec_mes,exogen=exogen,origin=origin,p=1,valmin=valmin,tolerance_wilks=tolerance_wilks,multisite_type="wilks",station=station)
## LOGIT-type Model
####model_multisite_wilks <- PrecipitationOccurenceMultiSiteModel(x=prec_mes,station=station,exogen=exogen,origin=origin,multisite_type="wilks",p=1)


obs_multisite <- prec_mes[,station]>=valmin

gen_multisite <- generate(model_multisite_wilks,exogen=exogen,origin=origin,end=end)
...
width = 480
height = width
if (plotccgamma_df) for (it in titles) {
		for (lag in c(0,1)) {
			
			title <- paste(it,"(Lag Y)",sep=" ")
			png <- paste(wpath,'plot/XXX_lagY_prec_wilks_RMAWGEN.png',sep='/')
			...
			ccgammaplot_df <- ccgammaplot(x=obs_mn,y=gen_mn,corx=NULL,xlab="observed",ylab="generated",title=title,season=TRUE,origin=origin,lag=lag,return.value=return.values[it],valmin=valmin,tolerance=tolerance_wilks,return.df=TRUE)
...
if (dw.spell.qqplot) {
	for (it in station) { 
		...
		png <- paste(wpath,'plot/qqplot_dw_spell_dry_prec_wilks_RMAWGEN_XXX.png',sep='/')  
		...
		png <- paste(wpath,'plot/qqplot_dw_spell_dry_prec_wilks__RMAWGEN_XXX_unseas.png',sep='/')  
		title <- "Quantile-Quantile dry spell length [days] at XXX"		
		...
		png <- paste(wpath,'plot/qqplot_dw_spell_wet_prec_wilks_RMAWGEN_XXX.png',sep='/')  
		...
		png <- paste(wpath,'plot/qqplot_dw_spell_wet_prec_wilks__RMAWGEN_XXX_unseas.png',sep='/')  
		title <- "Quantile-Quantile wet spell length [days] at XXX"		
...
if (nwetdayplot==TRUE) for (it in station) {
	...
	png <- paste(wpath,'plot/qqplot_nwetdays_prec_wiks_RMAWGEN_XXX.png',sep='/')  
